...
class CustomCollatorCLF(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        if "pairs" in features[0]:
            input_features["pairs"] = get_tensor_long(features[0]["pairs"])
            input_features["labels"] = get_tensor_long([features[0]["labels"]])
            input_features["user_embed_idxs"] = get_tensor_long([f["user_embed_idx"] for f in features])
            input_features["data"] = deepcopy(features[0]["data"]) # don't convert to cuda
