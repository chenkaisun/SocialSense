...
from torch.nn import Tanh, GELU, ReLU
...
class HGT(torch.nn.Module):
    def __init__(self, args, hidden_channels=-1, out_channels=-1, num_heads=2, num_layers=2, data=None):
        super().__init__()

        hidden_channels=args.g_dim#hidden_dim
        self.lin_dict = torch.nn.ModuleDict()
        self.dropout = args.dropout
        for node_type in data.node_types:
            self.lin_dict[node_type] = Linear(args.embedding_plm_hidden_dim, hidden_channels)
        # self.lin_dict[node_type] = Linear(-1, hidden_channels)

        self.convs = torch.nn.ModuleList()
        for _ in range(args.num_gnn_layers):
            conv = HGTConv(hidden_channels, hidden_channels, data.metadata(),
                           args.num_gnn_heads, group='sum')
    ...
    def forward(self, x_dict, edge_index_dict,edge_attr_dict=None):
        x_dict = {#.relu_()
            node_type: self.act(self.lin_dict[node_type](x))
        ...
        if self.dropout!=0:
            x_dict = {
                node_type: F.dropout(self.act(x), self.dropout, training=self.training)
                for node_type, x in x_dict.items()
            }
