...
def add_experiment_arguments(parser):
    ...
    parser.add_argument("--d_anno", default=1, type=int)
    parser.add_argument("--even_history", default=0, type=int)
    parser.add_argument("--rmv_sp_symbol", default=0, type=int)
    parser.add_argument("--swpp", default=0, type=int)
    parser.add_argument("--anonym", default=0, type=int)
    parser.add_argument("--use_gpu", default=1, type=int, help="Using gpu or cpu", )
    parser.add_argument("--eval_special", default=0, type=int, help="Using gpu or cpu", )
    parser.add_argument("--use_gnn", default=0, type=int, help="Using gpu or cpu", )
    parser.add_argument("--pred_label_category", default="", type=str, help="Using gpu or cpu", )
    parser.add_argument("--case_study", default=-1, type=int, help="Using gpu or cpu", ) # 0 for lurker, 1 for non-lurker

    parser.add_argument("--eval_verbose", default=0, type=int, help="Using gpu or cpu", )

    parser.add_argument("--pure_int", default=0, type=int, help="Using gpu or cpu", )
...
def add_data_arguments(parser):
    ...
    parser.add_argument("--custom_user_file", default=0, type=int)
    parser.add_argument("--custom_post_file_suffix", default="", type=str)
    parser.add_argument("--custom_post_file", default=1, type=int)
    parser.add_argument("--custom_suffix", default="", type=str)
    ...
    parser.add_argument("--user_attributes", default="12", type=str)  # 0 nothing 1 profile 2 history 3 id 4 userl 5 sc
    parser.add_argument("--text_only", default=0, type=int)  # 1 profile 2 history 3 id
    parser.add_argument("--pretrain",default="",type=str)  # 0 nothing 1 id and profile to history 2 social net
    parser.add_argument("--input_scheme",default="1",type=str)  # 0 nothing 1 profile and history 2 id and profile
    parser.add_argument("--wdbid",default="",type=str)  # 0 nothing 1 profile and history 2 id and profile but not history
    ...
    parser.add_argument("--dataset", default="CNN", type=str)  # CNN
    ...
    parser.add_argument('--task_setting', default=2, type=int)
...
def add_training_arguments(parser):
    ...
    parser.add_argument("--embedding_batch_size", default=32, type=int, help="Batch size for training.")
    parser.add_argument("--weighted_loss", default=0, type=int, help="Batch size for training.")
    parser.add_argument("--load_best_model_at_end", default=1, type=int, help="Batch size for training.")
...
def add_model_arguments(parser):
    ...
    parser.add_argument('--confidence_threshold', type=float, default=0.8)  # ca is cross attention b/w memory, cm is causal
    parser.add_argument('--encode_on_the_way', type=int, default=0)  # ca is cross attention b/w memory, cm is causal
    parser.add_argument('--no_orig', type=int, default=1)  # ca is cross attention b/w memory, cm is causal
    parser.add_argument('--use_value', type=int, default=0)  # ca is cross attention b/w memory, cm is causal
    parser.add_argument('--no_user_news', type=int, default=0)  # 2 means no testing edges
    parser.add_argument('--generate_label_cat', type=int, default=-1)  # 1 means task setting 1 (int), 2 means task setting 2, otherwise means both

    # memoryh

    parser.add_argument("--plm", default="deberta-base", type=str, metavar='N')  # default="base-uncased",
    parser.add_argument("--embedding_plm", default="deberta-base", type=str, metavar='N')  # default="base-uncased",

    parser.add_argument("--max_seq_len", default=512, type=int)
    parser.add_argument('--downstream_layers', default=["submodule_1", "gnn", "combiner", "user_embeddings","post_embeddings"], nargs='+')  # "pooler", "classifier"
    ...
    parser.add_argument("--activation", default="relu", type=str)  # gelu
    ...
    parser.add_argument('--num_gnn_heads', type=int, default=2, help='Number of final hidden units for graph.')
    ...
    parser.add_argument('--load_pretrained_ent_emb', default=1, type=int, help='freeze entity embedding layer')
...
def add_extra_arguments(parser):
    ...
    parser.add_argument("--verbose_eval", type=int, default=0)
...
def read_args():
    ...
    user_file, post_file, history_file, graph_file, user_embedding_file,id_to_index_file = [""] * 6
    splits = ["train", "dev", "test"]

    user_embedding_file = f'{subset_dir}embedding.bin'
    id_to_index_file = f'{subset_dir}id_to_index.json'
    graph_file = f'{subset_dir}users_followings_dict.json'
    ...
    user_file = f'{subset_dir}users_info_dict_with_chatgptanno_sc.json'
    history_file = f'{subset_dir}users_history_dict.json'
    post_file = f'{subset_dir}post_data.json'
    if args.config in ['response_pred', 'response_pred_sent','response_pred_sent_gnn',
                       'response_pred_by_gen', 'response_pred_by_gen_eval',"response_pred_by_gen_load_prt"] or args.config.startswith('response_pred_sent_gnn'):  # args.task_setting in [TASK_SETTINGS['response_pred'], TASK_SETTINGS['response_pred_sent']]: #args.personal_response_pred or
        ...
        if args.t_anno:
            train_file = f'{subset_dir}response_data_balanced_train_anno.json'
            # dev_file = f'{subset_dir}response_data_balanced_dev_anno.json'
        if args.d_anno:
            # train_file = f'{subset_dir}response_data_balanced_train_anno.json'
            dev_file = f'{subset_dir}response_data_balanced_dev_anno.json'

        test_file = f'{subset_dir}response_data_balanced_test_anno.json'
        # if args.use_anno:
        #     dev_file = f'{subset_dir}response_data_balanced_dev_anno.json'
        #     test_file = f'{subset_dir}response_data_balanced_test_anno.json'
        # else:
        ...
        if args.case_study==1:
            train_file = f'{subset_dir}train_anno_case_lurker.json'
            dev_file = f'{subset_dir}dev_anno_case_lurker.json'
            test_file = f'{subset_dir}test_anno_case_lurker.json'
        ...
        #         test_file = ''
        user_file = f'{subset_dir}users_info_dict_a.json' if args.anonym else f'{subset_dir}users_info_dict_with_chatgptanno_sc.json'
        history_file = f'{subset_dir}users_history_dict_a.json' if args.anonym else f'{subset_dir}users_history_dict.json'
        post_file = f'{subset_dir}post_data.json'
    ...
    elif args.config in ['response_pred_by_gen_prt']:
        train_file = f'{subset_dir}history_data_train.json'
        dev_file = f'{subset_dir}history_data_dev.json'
        test_file = f'{subset_dir}history_data_test.json'

        user_file = f'{subset_dir}users_info_dict.json' if not args.anonym else f'{subset_dir}users_info_dict.json'
        history_file = f'{subset_dir}users_history_dict.json' if not args.anonym else f'{subset_dir}users_history_dict.json'
        post_file = f'{subset_dir}post_data.json'
        ...
        test_file = f'{subset_dir}custom_files/response_data_balanced_custom_{args.custom_suffix}'
        test_file = f'{subset_dir}custom_files/{args.custom_test_file}'

        # user_file = f'{subset_dir}users_info_dict_custom_spreadsheet.json'
        # history_file = f'{subset_dir}users_history_dict_custom_spreadsheet.json'
        # post_file = f'{subset_dir}post_data_custom_spreadsheet.json'
        ...
        #     history_file = f'{subset_dir}users_history_dict_custom_spreadsheet.json'
        #     post_file = f'{subset_dir}post_data_custom_spreadsheet.json'
        if args.custom_post_file:
            post_file = f'{subset_dir}custom_files/post_data_custom_{args.custom_suffix}.json'
        if args.custom_user_file:
            user_file = f'{subset_dir}custom_files/users_info_dict_custom_{args.custom_suffix}.json'
            history_file = f'{subset_dir}custom_files/users_history_dict_custom_{args.custom_suffix}.json'
        # if args.custom_user_file:
        #     user_file = args.custom_post_file
    ...
    if args.swpp:
        # swap train and dev file
        test_file, dev_file = dev_file, test_file
    ...
    args.user_embedding_file=user_embedding_file
    args.id_to_index_file=id_to_index_file
    ...
    args.embedding_plm_hidden_dim = PLM_DIM_DICT[args.embedding_plm]

    if args.g_dim == -1:
        args.g_dim = args.plm_hidden_dim
        args.g_dim = args.embedding_plm_hidden_dim
    args.plm = PLM_DICT[args.plm]
    args.embedding_plm= PLM_DICT[args.embedding_plm]
