...
def setup_common(args, tokenizer, data=None): #concept_index_mapping=None,
    ...
    mkdir("model_states/")
    if args.use_wandb:
        args.wdbid = wandb.run.id

    if not len(args.model_path):
        if args.use_wandb and args.task_mode!="ret" and wandb.run.id is not None:
    ...
    print("\nSEED STATE in SETUP_COMMON before get model")
    ...
    # set_seeds(args)
    model = get_model(args, tokenizer, data)
    view_model_param(args, model)
    print("Model", model)

    # print current seeds without setting them
    print("\nSEED STATE in SETUP_COMMON after get model")
    print("random seed", random.getstate()[1][0])
    print("np seed", np.random.get_state()[1][0])
    print("torch seed", torch.random.get_rng_state())
    print("torch cuda seed", torch.cuda.random.get_rng_state())
    ...
    model, optimizer, args.start_epoch, args.best_dev_score = load_model_from_path(model, optimizer, args.load_model_path, args.use_gpu, args.device)
    ...
    if len(args.load_model_path.strip()):
        ...
        #
        print("if len(args.load_model_path.strip())")
        if "/" not in args.load_model_path and ".pt" not in args.load_model_path:
            cache_path=f"model_states/{args.load_model_path}.pt"
            if not os.path.exists(cache_path):
                model_data=wandb.restore(f"{args.load_model_path}.pt", run_path=f"INCAS/{args.load_model_path}")
    ...
    if args.use_wandb and args.task_mode!="ret":
...
def get_optimizer(args, model, downstream_layers, no_decay):
    ...
    if args.use_gnn and not args.encode_on_the_way:
        grouped_parameters = [
            {"params": [p for n, p in model.named_parameters()], 'weight_decay': args.weight_decay, "lr": args.lr},
        ]
    else:
        grouped_parameters = [
            {"params": [p for n, p in model.named_parameters() if
...
def get_scheduler(optimizer, scheduler_type, num_warmup_steps=None, num_training_steps=None, eta_min=0, T_max=None):
    if scheduler_type == "linear":
        ...
        scheduler =  torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=0.5, patience=100, verbose=True, min_lr=1e-4)
    else:
...
def get_tokenizer(plm, save_dir="tokenizer/", slow_connection=False, args=None):
    ...
    if args.use_special_tag == 1:
        tokenizer.add_special_tokens({'additional_special_tokens': ['[label]', '[PROFILE]', '[HISTORY]', '[POST]']})  # '[GOAL]','[SUBGOAL]' , '[LABEL_SEP]'
    elif args.use_special_tag == 2:
        tokenizer.add_tokens(['[label]', '[PROFILE]', '[HISTORY]', '[POST]'])  # '[UID]','[SN]',
    elif args.use_special_tag in [3,5,6]: #true
        tokenizer.add_tokens(['[label]', '[PROFILE]', '[HISTORY]', '[POST]','[UID]','[SN]'])  # '[UID]','[SN]','@user','<email>','<phone>','<url>'
    elif args.use_special_tag == 4:
        tokenizer.add_tokens(['[label]', '[PROFILE]', '[HISTORY]', '[POST]','[UID]','[SN]','@user','<email>','<phone>','<url>'])  # '[UID]','[SN]','@user','<email>','<phone>','<url>'
