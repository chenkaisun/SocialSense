...
# import wandb
def get_model(args, tokenizer, data, **kwargs):
    model = None
    ...
    if args.model_name == "ours_clf":
        ...
        if args.use_gnn: # == "ours_clf_gnn"
            model = FET_GNN(args, data)
        else:
            model = FET(args)
    ...
    plm_model = model
    if args.task_mode in ["clf"] and "plm" in model.__dict__.keys():
        plm_model = model.plm
    if "get_input_embeddings" in dir(plm_model):
        if plm_model.get_input_embeddings().weight.shape[0] != len(tokenizer):
